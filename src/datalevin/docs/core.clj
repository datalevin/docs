               ...
               (fn [sys]
                 (let [app (if dev?
                       ...
                       app (cond-> app wrap-handler wrap-handler)
                        ;; Limit thread pool for small memory instances (default is 50 max)
                       jetty-server (do (log/info "Starting server on port" (:port sys) "with max 20 threads")
                                        (jetty/run-jetty app {:port (:port sys)
                                                              :join? false
                                                              :max-threads 20
                                                              :min-threads 4
                                                              :max-idle-time 30000}))]
