...
(defn- oauth-link
  [href class label icon]
  [:a {:href href
       :hx-boost "false"
       :class (str class " flex items-center justify-center gap-2 text-white px-4 py-2.5 rounded-lg font-medium transition")
...
(defn login-page
  ...
  (let [token (force anti-forgery/*anti-forgery-token*)
        ...
        github-configured? (and (seq (:github-client-id req))
                                (seq (:github-client-secret req)))
        google-configured? (and (seq (:google-client-id req))
                                (seq (:google-client-secret req)))
        oauth-configured? (or github-configured? google-configured?)]
    (auth-shell "Login"
                 ...
                 (when oauth-configured?
                   [:div {:class "mb-6 space-y-3"}
                    (when github-configured?
                      (oauth-link "/auth/github" "auth-github" "Sign in with GitHub" github-icon))
                    (when google-configured?
                      (oauth-link "/auth/google" "auth-google" "Sign in with Google" google-icon))
